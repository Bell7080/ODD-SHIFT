...
## 2026-07-12 플레이어/랩터 고밀도 프록시 모델링
- 랩터 모델은 머리, 상하 턱, 좌우 눈, 이빨, 턱 관절, 목, 척추, 꼬리뼈, 허벅지, 발목, 중족골, 앞발 손가락, 발가락 마디와 발톱을 개별 파츠로 분리했다.
- 랩터의 엉덩이/가슴/종아리에는 `muscleWeight`를 가진 근육 블록을 추가해 이후 부위별 능력치, 피격, 진화형 AI 유전자 값과 연결할 수 있게 했다.
- 인간 모델도 머리, 턱, 코, 목, 쇄골, 척추, 어깨, 팔꿈치, 손목, 골반, 무릎, 발목, 손가락, 발가락과 가중치형 근육 블록을 별도 파츠로 확장했다.
- 이번 작업에서는 임시 테스트용 코드나 인게임에 포함되지 않는 더미 파일을 추가하지 않았다.
