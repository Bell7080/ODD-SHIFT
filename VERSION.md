...
**시간대 진행 잠금을 해제하고 위험체 배치와 버튼 타이포그래피를 정돈했다.**

- 씬 재시작 뒤에도 팝업 잠금값이 남아 점심 업무창을 다시 열지 못하던 문제를 수정했다.
- 위험체의 에셋 원점 대신 실제 외형 하단을 구금 칸 바닥에 맞춰 발을 기준으로 서게 했다.
- 버튼의 색면 배경을 제거하고 글자를 키웠으며 보라·청록 네온 호버 반응으로 통일했다.

## v0.3.2 — 2026-08-17

**사고에 대응할 기회와 부서 사기, 드래그 편성, 로그 필터를 추가해 관리 피드백을 완성했다.**

- 영업 마감 시 몽환 70 이상 위험체가 있으면 경고 팝업을 열고 정비자원 25 또는 PORT 30으로
  몽환을 낮추거나 위험을 감수하고 마감할 수 있게 했다.
- 부서 사기 수치를 추가해 손님 업무 성공·직원 채용은 사기를 높이고 직원 사망은 크게
  낮추도록 했다. 현재 사기는 위험체의 일일 재료 생산 효율에 반영된다.
- 브리핑의 선택된 위험체 카드를 다른 선택 카드 위치로 드래그해 실제 출전 순서를 바꿀 수
  있게 했다. 짧게 누르는 기존 선택·해제 조작도 유지한다.
- 관제실에서 날짜가 기록된 전체 운영 기록을 열고 전체/운영/사고 필터로 분류해 볼 수 있게
  했다. 휴게실에는 현재 사기와 생산 효율을 함께 표시한다.

## v0.3.1 — 2026-08-17

**관리 실패의 실제 결과와 다중 구금·편성 UI를 연결해 운영 선택이 전투까지 이어지게 했다.**

- 시설 수용량에 맞춘 최대 여섯 개 구금 칸을 만들고 각 위험체의 몽환 수치와 담당자를
  함께 표시했다. 직원은 원하는 개체 칸에 독립적으로 드롭할 수 있다.
- 직원 드래그 중 몽환 감소, 포트 보너스, 전투 보정 예상치를 표시해 적성 배치 결과를
  놓기 전에 비교할 수 있게 했다.
- 몽환 70 이상에서 시설 피해, 90 이상에서 직원 사망과 위험체 탈출이 발생하도록 사고를
  구현하고 모든 결과를 관제실 운영 로그에 표시했다.
- 전투 브리핑에서 로스터 카드를 눌러 최대 3체 또는 시설 확장 후 6체의 출전 순서를 직접
  편성할 수 있게 했다.
- 2대2 태그매치에서 양측 두 번째 개체의 체력을 동시에 표시하고 지원 공격과 반격을 같은
  턴에 처리하도록 전투 계산을 확장했다.

## v0.3.0 — 2026-08-17

**연구소를 3층 사이드뷰 운영 공간으로 바꾸고 포트 중심의 반복 영업·관리 루프를 연결했다.**

- 관제도를 세 층의 바닥을 옆에서 보는 단면 구조로 다시 그리고 굵은 외곽선·잘린 모서리·
  넓은 보라/청록 색면으로 화면 전반의 페이퍼컷 UI를 강화했다.
- 직원 마커를 구금실로 드래그해 위험체를 관리하고 몽환 스트레스를 낮출 수 있게 했다.
  담당자가 없는 채 하루를 넘긴 위험체는 스트레스가 크게 증가한다.
- 포트 재화를 추가하고 직원 고용과 구금소 증설에 연결했다. 회유한 위험체는 다음 아침
  연구실 임시 보호 대상으로 나타나 구금 또는 방생할 수 있다.
- 전투 보상 정산 후 같은 날 손님 접수로 돌아오며, 원하는 만큼 반복한 다음 `영업 마감`으로
  다음 날을 시작하도록 하루 흐름을 변경했다.
- 날짜·페이즈 팝업에 닫기 버튼과 바깥 영역 터치 닫기를 추가했다.
- 전투 직전 화면을 슬롯 머신형 브리핑으로 개편해 움직이는 적 미리보기, 체력 바,
  1대1/2대2 조건과 큰 긴급 기상 횟수를 한 화면에 표시했다.

## v0.2.2 — 2026-08-17

**모바일 표시와 전투 구도, 연구소 운영 화면의 정보 표시를 바로잡았다.**

- 1280×720 논리 해상도를 모바일 뷰포트 안에 맞추도록 FIT 스케일과 동적 뷰포트
  높이, 터치 제스처 차단을 적용했다.
- 전투의 적을 오른쪽 상단으로 올리고 아군은 왼쪽 하단에 두어 대각선으로 마주 보는
  실루엣이 되도록 퍼펫과 발판을 함께 재배치했다.
- 운영 화면 구금실에 현재 로스터 위험체의 실제 퍼펫을 다시 표시했고, 날짜·페이즈
  업무창은 평소에 숨겨 두었다가 화면 하단 버튼을 눌렀을 때만 열리도록 바꿘었다.
