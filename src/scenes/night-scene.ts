...
export class NightScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    makeText(this, 24, 80, `${gameState.selectedGuestLabel}의 꿈속으로 들어갑니다…`, 'heading', {
      fontSize: '16px',
      color: '#f2e9ff',
    });
    makeText(this, 24, 130, `침입 개체 수: ${plan.enemyCount}마리${plan.tagTeam ? ' (태그매치)' : ''}`, 'body', {
      fontSize: '15px',
      color: '#e8dcff',
    });
    makeText(this, 24, 160, `악몽: ${plan.nightmare.name} — ${plan.nightmare.description}`, 'body', {
      fontSize: '15px',
      color: '#e8dcff',
    });
    makeText(this, 24, 190, `응급 각성 가능 횟수: ${plan.wakeUps}회`, 'body', { fontSize: '15px', color: '#e8dcff' });

    makeText(this, 24, 240, '오늘 밤 조우할 위험체', 'heading', { fontSize: '16px', color: '#f2e9ff' });
    let y = 270;
    gameState.encounterQueue.forEach((entity) => {
      makeText(this, 24, y, `${entity.name} · 위협도 ${entity.threatTier} · HP ${entity.combat.maxHp}`, 'body', {
        fontSize: '13px',
        color: '#b7a6dd',
      });
      y += 20;
