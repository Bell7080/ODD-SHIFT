...
export class EveningScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(24, 60, '저녁 — 낮 동안 모은 자원으로 시설을 강화하거나 직원을 영입합니다.', {
      fontSize: '14px',
      color: '#c7b8ee',
    });

    this.add
      .text(24, 110, `정비자원 ${UPGRADE_COST} 소모 · 1구역 수용실 안정도 강화`, {
        fontSize: '13px',
        color: '#0a0710',
    ...
    this.add
      .text(24, 150, `연구자원 ${HIRE_COST} 소모 · 신입 직원 영입`, {
        fontSize: '13px',
        color: '#0a0710',
        backgroundColor: '#7fd4a2',
        padding: { x: 10, y: 6 },
    ...
    this.add.text(24, 200, '오늘 밤 자원 운영을 마쳤다면, 손님을 맞이할 차례입니다.', {
      fontSize: '13px',
      color: '#9683c4',
    });
